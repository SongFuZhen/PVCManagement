...
namespace BlueCarGpsWeb.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginViewModel model, string returnUrl)
        {
            ...
            User user = us.Auth(model.LoginField, model.Password);
            if (user != null)
            {
                ...
                claims.Add(new Claim(ClaimTypes.Role, ((int)user.roleTypeEnum).ToString()));

                var identity = new ClaimsIdentity(claims, "Login");
                ClaimsPrincipal p = new ClaimsPrincipal(identity);
                System.Web.HttpContext.Current.User = p;
            }
            if(returnUrl!=null && returnUrl.Trim()== "/Account/LogOff")
            {
                returnUrl = "/";
            }
